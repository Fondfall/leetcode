 ...
 * 击败第 i 个对手会使你的经验 增加 experience[i]，但会将你的精力 减少energy[i] 。
...
public class MinNumberOfHours_2383 {
    public int minNumberOfHours(int initialEnergy, int initialExperience, int[] energy, int[] experience) {
        ...
        for (int i = 0; i < length; i++) {
            if ((diff = initialExperience - experience[i]) <= 0) { //如果经验值不够，需要训练
                hours = hours - diff + 1;
                initialExperience = initialExperience - diff + 1;
            }
            initialExperience += experience[i];

            if ((diff = initialEnergy - energy[i]) <= 0) {//精力值不够，需要训练
                hours = hours - diff + 1;
                initialEnergy = initialEnergy - diff + 1;
            }
